...
export async function stageImageWithGemini(
  ...
  try {
    ...
    // Initialize the new Gemini image generation client
 ...
 */
export async function stageImageWithGeminiRetry(
  imageUrl: string,
  options: StagingOptions
): Promise<StagingResult> {
  return retryWithBackoff(async () => {
    return stageImageWithGemini(imageUrl, options);
  });
}
...
export async function stageImagesInBatch(
  ...
  for (let i = 0; i < images.length; i += concurrency) {
    ...
    const batchPromises = batch.map(async ({ url, options }) => {
      try {
        return await stageImageWithGeminiRetry(url, options);
      } catch (error) {
